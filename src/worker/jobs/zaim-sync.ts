 ...
 * 1日2回になった時点（#165）で、11:30 の巡回は前夜に更新できた口座を
 * 「更新できなかった」と判定し、23:30 の巡回で解消する——という警告と復旧の往復を毎日生む。
 *
 ...
 * 設定を読めないため。20時は `deploy/systemd/aide-zaim-sync.timer` の 23:30 だけを拾い、
 * 昼の 11:30 を外せる境目として置いている（タイマーの時刻を変えるならここも見直す）。
 *
