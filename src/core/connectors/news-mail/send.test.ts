...
const dir = await mkdtemp(join(tmpdir(), "aide-news-mail-send-test-"));
...
const { sendNewsMail } = await import("./send.ts");
const { NEWS_MAIL_LOG_PATH } = await import("./log.ts");
...
function baseInput(idempotencyKey: string) {
  return {
    ...
    bodyText: "本文です",
    bodyHtml: "<table><tr><td>本文です</td></tr></table>",
    articleCount: 12,
  };
}
...
async function readLog(): Promise<unknown[]> {
  return JSON.parse(await readFile(NEWS_MAIL_LOG_PATH, "utf8")) as unknown[];
}

describe("sendNewsMail", () => {
  it("成功: messageIdを返し、ログに記録される", async () => {
    const outcome = await sendNewsMail(
      credentials,
      addresses,
      baseInput("send-1"),
    ...
    const log = (await readLog()) as { ok: boolean; messageId: string | null; articleCount: number }[];
    const entry = log.find((e) => e.messageId === "msg-1");
    assert.ok(entry);
    assert.equal(entry.ok, true);
    assert.equal(entry.articleCount, 12);
  });
  ...
  it("同じidempotencyKeyで再送すると、Gmailへ送らずduplicated:trueを返す", async () => {
    ...
    const first = await sendNewsMail(credentials, addresses, baseInput("send-2"), fetchImpl);
    assert.equal(first.ok, true);
    const second = await sendNewsMail(credentials, addresses, baseInput("send-2"), fetchImpl);
    assert.deepEqual(second, { ok: true, messageId: "msg-2", duplicated: true });
    assert.equal(sendCalls, 1);
  });

  it("rejected（Gmailが拒否）は記録を消し、次回同じキーで再試行できる", async () => {
    const rejected = await sendNewsMail(
      credentials,
      addresses,
      baseInput("send-3"),
    ...
    const retried = await sendNewsMail(
      credentials,
      addresses,
      baseInput("send-3"),
  ...
  it("failed（送信されたか不明）は記録を残し、次回同じキーはconflictになる", async () => {
    const failed = await sendNewsMail(
      credentials,
      addresses,
      baseInput("send-4"),
