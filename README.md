...
**単機能ツールを出している唯一の例外がZaimへの登録**（`aide_zaim_master` / `aide_zaim_payment`。aide#135）。
...
- **ハッシュにするのは記録に中身を残さないため。** `data/zaim-payments.json` は
  「`requestId`・`money_id`・時刻の3つだけ」という方針で、平文のキーにすると支出の中身が溜まる
