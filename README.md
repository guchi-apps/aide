...
| 更新できなかった口座の警告の抑制 | 6時間 → 24時間 | `src/worker/notify.ts` の `STALE_ACCOUNTS_SUPPRESSION_WINDOW_MS` |

最後の1つは、抑制を失敗通知（6時間のまま）と分けたもの。**この警告は実行のたびに必ず再判定される**ため、窓を6時間のままにすると実行回数を増やしただけ同じ口座の警告が増える。直すのはZaim側の連携設定で、日に何度知らせても取れる行動は変わらない。
...
**「一部失敗」だけは窓が24時間で、失敗の6時間とは別**（`STALE_ACCOUNTS_SUPPRESSION_WINDOW_MS`）。こちらはジョブが走るたびに必ず再判定されるため、`zaim-refresh` の実行回数を増やすと窓のぶんだけ警告も増える（#165）。直すのはZaim側の連携設定で、日に何度知らせても取れる行動は変わらない。
