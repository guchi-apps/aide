...
照合するパスワードは同じだが、Cookie1つで完結させている（`src/web/session.ts`）。OAuthに載せると
画面を開くたびに認可コードの往復が要る。**Cookieには署名だけを入れ、サーバー側に状態を持たない。**
鍵はパスワードそのものから導くため、パスワードを変えれば発行済みのCookieは自動的に無効になる。
