...
| | 経路 | 資格情報 | 持つ操作 |
|---|---|---|---|
| `aide_create_issue`（aide#50） | ClaudeアプリからのGitHub Issue起票 | `AIDE_GITHUB_ISSUE_TOKEN`（取得用とは別のPAT） | 作成のみ |
| `POST /api/zaim/payment`（aide#37） | 個人アプリからZaimへの支出登録 | Zaim APIの OAuth 1.0a（巡回の storage state とは別） | 作成のみ |

Zaimへの登録は、car-care（給油記録）とasset-manager（レシート由来の支出）から要望が出たもの。
**Zaimの資格情報を持つのをAIDEだけにする**ため、書き込む口もAIDEへ1本化している
（各アプリがそれぞれZaimクライアントを持つと、読み取り側で起きた重複（asset-manager#191）を
書き込み側でも作ることになる）。詳細は[個人アプリ向けのZaim登録API](#個人アプリ向けのzaim登録api)。
