...
| | |
|---|---|
| 残すもの | 時刻・メソッド・ツール名・名乗ったクライアント名とバージョン・成否・失敗理由1行・所要ミリ秒 |
...
**上限を超えた分は、接続確認（`ping`）と一覧の取得から先に捨てる。** 単純に古い順で捨てると、
Claudeが定期的に投げてくるぶんだけが残り、いちばん見たいツールの呼び出しが押し出される。
同じ理由で画面でもそれらの行は既定で畳んであり、チェックを入れたときだけ同じ表に混ざる。
